...
export class DateUtil {
    ...
    /**
     * 배열에서 가장 이른 날짜의 복사본을 반환합니다. 빈 배열이면 `Invalid Date`를
     * 반환해 호출자가 별도로 값 없음 여부를 판단할 수 있게 합니다.
     */
    ...
    /**
     * `YYYYMMDD` 또는 `YYYYMMDDHHmm` 형식의 문자열을 로컬 시간대 Date로 변환합니다.
     * 시간이 없으면 00:00으로 채웁니다. 형식 길이가 다르면 예외를 던지지만,
     * 달력에 없는 날짜 보정 여부는 JavaScript Date 동작을 그대로 따릅니다.
     */
    static fromYMD(dateString: string): Date {
        ...
        const month = parseInt(dateString.substring(4, 6), 10) - 1 // JS Date의 월은 0부터 시작합니다.
    ...
    /**
     * 배열에서 가장 늦은 날짜의 복사본을 반환합니다. 빈 배열이면 `Invalid Date`를
     * 반환합니다.
     */
    ...
    /**
     * Date 객체의 로컬 날짜 부분을 `YYYYMMDD` 문자열로 변환합니다.
     */
    static toYMD(date: Date): string {
        const year = date.getFullYear()
        const month = (date.getMonth() + 1).toString().padStart(2, '0') // JS Date의 월은 0부터 시작합니다.
