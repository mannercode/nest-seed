 ...
 * HTTP 전용 예외 로거입니다.
 *
 * NestJS의 예외 필터 선택은 공식 문서에 없는 내부 동작에 기댑니다. 등록된
 * 필터 배열을 역순으로 훑으며 `.find`로 가장 먼저 일치하는 필터 하나만
 * 실행합니다. `@Catch()` (타입 없음)는 무조건 일치하고, `@Catch(Type)`은
 * `exception instanceof Type` 일 때 일치합니다. 이 알고리즘은 NestJS 버전에
 * 따라 바뀔 수 있습니다. 그래서 등록 순서에 의존하지 않도록 이 필터를 상속
 * 하는 방식으로 확장합니다.
 *
