...
// 성공 로그와 예외 로그가 같은 기준 시각으로 처리 시간을 계산하게 합니다.
// 인터셉터는 요청 진입 시각을 알고, 예외 필터는 실패 시점을 아니까 시작 시각을
// WeakMap으로 공유합니다.
// 시작 시각을 `WeakMap`에 두면 `express.Request` 객체를 직접 수정하지 않아도
// 됩니다. 전역 타입 확장도 필요 없고, 같은 필드 이름을 쓰는 다른 미들웨어와
// 부딪힐 일도 없습니다.
