 ...
 * `[REDACTED]`로 바꾼 깊은 복사본을 반환합니다. HTTP 요청·응답 본문이 로그에
 * 닿기 전에 가리는 용도입니다.
 *
 * 객체끼리 서로를 참조하는 자가 순환 그래프도 안전하게 처리합니다. 같은
 * 객체를 다시 만나면 끝없이 재귀하지 않고 `[CIRCULAR]`로 축약합니다. 예를 들어
 * 에러 로깅 경로에 포함되는 `error.cause = error` 같은 사슬이 그렇습니다.
 */
