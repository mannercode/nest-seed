...
/**
 * refresh 토큰 회전과 재사용 탐지를 갖춘 JWT 인증 서비스입니다.
 *
 * Redis 저장 구조 (인스턴스마다 `prefix`로 분리):
 *
 *   {prefix}:{familyId}:token:{tokenId}   → JSON `{ hash, familyId }`, TTL = refresh 만료 시각
 *   {prefix}:{familyId}:family            → 살아 있는 tokenId의 SET, TTL = refresh 만료 시각
 *   {prefix}:user:{userId}:families       → 그 사용자의 familyId SET, TTL = refresh 만료 시각
 *
 * - 해시만 저장합니다. `SHA-256(refreshToken)`만 Redis에 두고, 원본 JWT는
 *   클라이언트에만 둡니다. Redis 덤프가 유출되어도 그대로 사용할 수 있는 토큰이
 *   공격자에게 직접 노출되지 않습니다.
 * - refresh가 성공할 때마다 토큰을 회전합니다. 쓴 토큰 항목은 지우고, 같은
 *   family 안에서 새 토큰 ID를 발급합니다.
 * - 이미 회전된 토큰(토큰 항목은 사라졌지만 family는 살아 있는 상태)이
 *   다시 제출되면 도난으로 간주합니다. family 전체를 무효화해 정상 사용자와
 *   공격자 모두 더는 refresh 하지 못하게 합니다.
 * - 서명 알고리즘을 고정하고 `iss` / `aud` 클레임을 검증합니다. 알고리즘 혼동
 *   공격이나, 같은 secret으로 다른 서비스가 발급한 토큰을 거릅니다.
...
export class JwtAuthService {
    ...
    async refreshAuthTokens(refreshToken: string, context?: EventContext): Promise<JwtAuthTokens> {
        ...
        // 다른 호출 가운데 우리가 먼저 지웠는지 알 수 있습니다. 그 경쟁에서
        // 경쟁에서 실패한 호출은 이미 회전된 토큰을 다시 제출한 경우와 구분할 수 없으므로,
        // 재사용 탐지가 family 전체를 무효화합니다.
     ...
     * family의 키가 같은 슬롯에 모이게 만들기 위해서입니다. token 키와
     * family 키를 한 MULTI 파이프라인 안에서 함께 다루는데, Redis
     * Cluster는 슬롯이 다른 키를 한 트랜잭션에 묶으면 `CROSSSLOT`으로
     * 거절합니다.
     *
    ...
    private async consumeToken(tokenId: string, familyId: string): Promise<boolean> {
        // 결과의 첫 항목이 DEL 응답이다 (`[err, count]`). 카운트가 1이상이면
        // 동시에 들어온 소비자 중 우리가 먼저 지운 것입니다. 0이면 다른
        // 워커가 이미 지운 뒤입니다.
        //
        // ioredis의 `multi().exec()`는 transaction이 중단되면 (예: 연결
        // 끊김) `null`을 반환합니다. 이 경우에는 DEL이 실제로 실행됐는지
        // 알 수 없습니다. 그래서 짐작 없이 그대로 throw 합니다. false를 반환하면
        // 일시 오류였을 때도 family 전체를 무효화하고, true를 반환하면 같은 토큰에서
        // 유효한 새 토큰이 두 개 발급될 수 있습니다. throw 하면 5xx가 되어
        // 클라이언트가 다시 시도하고, family는 그대로 남습니다.
    ...
    private async emit(event: SecurityEvent) {
        ...
        try {
            ...
            // 이 훅이 실패해도 인증 자체는 계속 진행합니다. 로그만 남깁니다.
