...
/**
 * 추가만 일어나는 도메인용 리포지토리 기반 클래스이다. 노출 면을 일부러
 * 작게 제한했다. 이 도메인의 모든 사용자가 공통으로 필요한 책임은 두 가지뿐이라
 * 그렇다.
 *
 ...
 * 검증, 조회 패턴이 달라서, 공통 조회 API를 미리 고정하는 것은 이른 추상화이다.
...
export abstract class AppendOnlyRepository<Doc> implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        // 에러를 낸다. 단위 테스트처럼 빠르게 초기화를 반복하는 환경에서 자주
        // 보이는 증상이다. 모듈 초기화 시점에 미리 만들어 두면 경합이 사라진다.
