...
/**
 * MongoDB의 중복 키 에러(E11000)인지 판별한다. unique 인덱스 충돌이 발생했을 때
 * 안전하게 409 Conflict로 매핑하기 위해 사용한다.
 */
...
export class QueryBuilder<T> {
    ...
    addRegex(
        ...
        if (value) {
            // 기본값(부분 문자열 + 대소문자 무시)은 인덱스를 활용하지 못한다. 컬렉션
            // 전체 스캔이 된다. 인덱스를 활용하려면 두 옵션을 함께 켠다.
