...
/**
 * 추가만 일어나는 도메인용 스키마 기반 클래스입니다. 감사 로그, 이벤트 로그,
 * 변경 불가 이력처럼 수정·삭제가 일어나지 않는 모델에 사용합니다.
 *
 * `CrudSchema`와 달리 `updatedAt`과 `deletedAt` 필드가 없습니다. append-only에서는
 * 의미가 없는 값이라서입니다. `createdAt`만 노출합니다.
 *
 * `createAppendOnlySchema`가 스키마 단계에서 모든 수정·삭제 연산을 예외로 막습니다.
 * 모델 메서드를 직접 부르는 우회 경로까지 같이 차단됩니다.
 */
...
export function createAppendOnlySchema<T>(cls: Type<T>): Schema<T> {
    ...
    // `save()`는 신규 삽입에만 허용합니다. 기존 문서를 다시 저장하면 예외를 던집니다.
