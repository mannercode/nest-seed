 ...
 * 반환한 값을 그대로 넘깁니다. 스키마가 형 변환을 하지 않으면, boolean이나
 * number를 요청해도 문자열이 반환될 수 있습니다(예: `process.env`에서 그대로
 * 읽힌 `"true"`). 이 기반 클래스는 호출자에게 약속한 타입을 항상 보장합니다.
 * 문자열로 들어온 boolean이나 number는 여기서 직접 변환하고, Joi 같은 상위
 * 검증이 이미 변환했다면 그 결과를 그대로 통과시킵니다.
 */
