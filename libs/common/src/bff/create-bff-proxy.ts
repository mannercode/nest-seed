...
type BffRequest = Request & { cookies: { get(name: string): { value: string } | undefined } }
type BffOptions = {
    accessCookie: string
    refreshCookie: string
    authPrefix: 'admins' | 'users'
    apiBaseUrl: () => string
...
export function createBffProxy({
    ...
    async function proxy(request: BffRequest, context: RouteContext): Promise<Response> {
        if (!hasSameOrigin(request.method, request.headers, new URL(request.url).protocol)) {
        ...
        if (isLogin && upstream.ok) {
            ...
            const response = new Response(null, { status: 204, headers: PRIVATE_RESPONSE_HEADERS })
    ...
    async function readBody(request: BffRequest): Promise<ArrayBuffer | undefined> {
    ...
    async function copyResponse(upstream: Response): Promise<Response> {
    ...
    function jsonResponse(body: unknown, status: number): Response {
        return Response.json(body, { headers: PRIVATE_RESPONSE_HEADERS, status })
    }

    function setAuthCookies(response: Response, tokens: AuthTokens): void {
        setCookie(response, ACCESS_COOKIE, tokens.accessToken, tokenExpiry(tokens.accessToken))
        setCookie(response, REFRESH_COOKIE, tokens.refreshToken, tokenExpiry(tokens.refreshToken))
    }
    ...
    function setCookie(response: Response, name: string, token: string, expiresAt: number): void {
        response.headers.append(
            'Set-Cookie',
            `${name}=${token}; Path=/; HttpOnly; SameSite=Lax; Expires=${new Date(expiresAt * 1000).toUTCString()}${secureCookies ? '; Secure' : ''}`
        )
    }
