...
describe('vi.spyOn', () => {
    it('모듈 함수에 감시자를 건다', () => {
        const mockFunc = vi.spyOn(Module, 'getGreeting')
    ...
    it('모듈 함수를 mock 구현으로 대체한다', () => {
        ...
        const mockFunc = vi.spyOn(Module, 'getGreeting').mockReturnValue('Mocked Value')
    ...
    it('클래스 인스턴스 메서드를 mock 구현으로 대체한다', () => {
        ...
        const mockFunc = vi.spyOn(localObj, 'getHello').mockReturnValue('Mocked Value')
    ...
    it('클래스 getter를 mock 구현으로 대체한다', () => {
        const localObj = new Module.HelloClass()
        const mockFunc = vi.spyOn(localObj, 'value', 'get').mockReturnValue(1000)
