...
test('Vitest lifecycle은 worker 자원 준비, 테스트별 정리, 종료를 연결한다', async () => {
    const events = []
    const mongo = {
        async close() {
    ...
    try {
        ...
        if (previousWorkerId === undefined) delete process.env.VITEST_POOL_ID
        else process.env.VITEST_POOL_ID = previousWorkerId
    }
...
test('Vitest lifecycle의 선택 callback과 부분 초기화 정리는 없어도 안전하다', async () => {
