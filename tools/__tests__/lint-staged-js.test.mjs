...
test('lint-staged preserves spaces and apostrophes while partitioning JavaScript files', async (t) => {
    ...
    const argumentLog = join(repository, 'pnpm-arguments.jsonl')
    ...
    await chmod(join(mockBin, 'pnpm'), 0o755)
    ...
    assert.deepEqual(calls[2].slice(0, 3), ['exec', 'prettier', '--write'])
    assert.deepEqual(
        new Set(calls[2].slice(3)),
        new Set([
