# `.npmrc`, 이 Dockerfile, 루트 package/lockfile과 모든 워크스페이스 manifest로 `node_modules` 베이스 이미지를 만든다.
# `ensure-deps-image.sh`가 이 입력들의 해시를 태그로 쓰므로, 앱 이미지 빌드는 의존성 입력이 바뀔 때만 npm install 비용을 다시 낸다.
