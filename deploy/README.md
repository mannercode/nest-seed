...
| `verify.sh`   | compose up → [../apps/api/api-docs/run.sh](../apps/api/api-docs/run.sh) 실행 → down |
...
| `DEPS_TAG`             | 필수                                  | 빌드에 사용할 deps 이미지 태그                  |
...
`deploy/compose.yml`은 service의 `env_file`로 root의 `.env.infra`와 `.env.api`를 직접 가져온다. 추가로 deploy 시점에 덮어쓸 값(`NODE_ENV=production`, `LOG_DIRECTORY=/app/logs` 등)만 compose.yml의 `environment`에 둔다.
