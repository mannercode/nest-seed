...
P1은 선택한 기술 예제의 정합성·복구 보장을 해치는 문제, P2는 입력·검증·실행 재현성 문제, P3는 설명·유지보수 개선이다. **범위 결정** 항목은 자동으로 구현할 목록에 포함하지 않는다. 코드로 확인한 경로와 실제로 실행한 재현도 구분했다.
...
먼저 부모 삭제와 상영 생성의 동시 정합성을 이 시드에서 보여 줄 예제에 포함할지 정해야 한다. 포함한다면 같은 부모의 생성·삭제가 공통 직렬화 지점을 거치게 하고 기존 불변식을 검증해야 한다. 제외한다면 지원 범위와 보장을 명확히 좁히는 것도 가능하다. 이 문제를 발견했다는 이유만으로 영화 삭제·업로드·상영 생성 전체를 조율하는 복구 체계까지 필수 과제로 삼지는 않는다. 현재 계약의 경합이 해결되지 않았다는 사실은 유지한다.
...
이 지적은 이미 구현한 완료 재시도의 상태 판정을 바로잡는 범위다. 파일 처리의 모든 장애 조합을 복구하는 범용 workflow나 outbox 도입까지 요구하지 않는다.
...
시드에서는 1시간 보존도 충분한 선택이다. 먼저 조회 가능한 기간, 만료 후 응답, 같은 키의 재요청 의미를 명시하는 정도를 추천한다. 영구 결과 저장이나 추가 복구 체계를 기본 과제로 삼지 않는다. 더 긴 결과 조회를 예제로 선택할 때 종결 결과의 소유권과 접수·조회 수명을 함께 설계하면 된다. 실제 보존 만료는 재현하지 않았으며, 현재 테스트는 즉시 조회의 ready/pending만 다룬다.
...
이는 공개 상태 예제와 추천 예제를 조합할 때 생기는 도메인 정책의 차이다. 추천 후보를 공개작으로만 제한해야 한다고 이 검토에서 결정하지 않는다. 두 예제가 공유할 공개 정책을 정하거나, 각 예제의 지원 범위를 설명하면 된다. 선택한 정책을 넘어 상영 편성·추천·공개 상태의 모든 조합을 구현할 필요는 없다. 이번에는 정적 경로를 확인했다.
...
현재 동기 race가 주장하는 보장을 지키려면 자원 그룹별 분산을 확인해야 한다. 비동기 상영은 HTTP 접수 복제본과 실제 Restate 실행 복제본이 다르다. 실제 실행 복제본의 경쟁까지 검증하려면 실행 지점 관측과 상영·티켓 최종 상태 대조를 보완하고, 현재 검증 범위만 유지하려면 테스트의 설명을 좁힌다. 이 추가 관측 범위는 별도 선택이다. 반복 횟수를 늘리는 것만으로 기존 판정의 빈틈이 해결되지는 않는다.
...
우선 개선은 **선택한 기술 예제의 실패·재시도·입력 계약과 검증기의 정확성, 실행 재현성**에 한정한다. 1·5·6번은 예제로 다룰 범위를 정한 뒤 필요한 만큼만 구현한다. 기능 하나가 대표 기술을 충분히 보여 준다면 영화 예매 서비스 전체를 완성하기 위한 정책·상태·복구 체계를 덧붙이지 않는다. 기능 수정과 새 테스트 파일 작성은 이번 검토에 포함하지 않았다.
