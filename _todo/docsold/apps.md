...
각 앱의 Route Handler가 쿠키·API 요청 전달·갱신을 처리한다. access·refresh token은 HttpOnly cookie에 보관하고 쿠키 만료는 각 JWT의 `exp`를 따른다. 액세스 인증이 실패하면 리프레시 토큰을 회전한 뒤 원 요청을 한 번 재시도한다. 응답을 캐시하지 않고 body 크기를 제한한다. catch-all proxy의 일부 auth 경로 차단만으로 권한을 보장하지 않으며, 최종 인가는 API guard가 담당한다.
