...
- `application/booking.spec.ts`의 집계 누락·타인 선점, `application/recommendation.spec.ts`·`view/home.spec.ts`의 추천 제외 대상, `core/movies-publish.spec.ts`의 CAS 실패·공개 상태 준비도 같은 구조로 맞춘다. `core/tickets.spec.ts`·`core/purchase-records.spec.ts`의 판매·구매 기록 준비와 `infrastructure/payments.spec.ts`의 저장 오류 주입도 대상이다.
...
- [tickets.spec.ts](../../apps/api/src/__tests__/core/tickets.spec.ts)의 `sellForPurchase` 성공 검사는 `every`만 사용해 빈 배열도 통과한다. 요청한 티켓 ID·반환 개수·구매 귀속을 함께 검사한다. 다른 테스트의 DB 검증을 없애지 않는다.
- `core/admin-auth.spec.ts`·`core/user-auth.spec.ts`의 refresh 성공 검사는 이전 토큰과 다르다는 사실만 단언해 undefined도 통과한다. 응답의 토큰 필드와 교체 결과를 함께 검사한다.
