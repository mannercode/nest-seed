...
진행할 항목은 API-1·API-2·API-3이다. API-4와 배치·네이밍의 선택적 정리는 보류한다. 작업 범위와 읽기 부담의 기준은 [진행할 작업](README.md)을 따른다.
...
### API-1 — 진행 / 확신 높음: 같은 좌석 좌표의 티켓을 중복 생성·판매할 수 있다
...
### API-2 — 진행 / 확신 높음: 허용된 0좌석 상영을 예매 화면에서 조회하면 500이다
...
### API-3 — 진행 / 확신 높음: 결제의 부분 unique index를 멱등성 조회가 사용하지 못한다
