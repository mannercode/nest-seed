...
### L1 — P2 / 확신 높음: NATS 종료 후 다음 handler가 새로 실행된다
...
- 최소 개선: 종료 시 각 state의 등록 handler를 비우고 소비 task를 추적하여 종료 훅이 진행 중 task의 끝을 관찰하게 한다. 정상 경로와 종료 barrier 경로를 기존 nats spec에서 검증한다. 새 메시지 재시도나 별도 shutdown framework는 필요 없다.
- 테스트 공백: 기존 destroy 테스트는 종료 뒤 새 publish만 확인하여 이미 실행 중인 메시지의 나머지 handler를 놓친다.

### L2 — P2 / 확신 높음: SSE 문자열이 UTF-8 바이트 경계에서 손상된다
...
### L3 — P2 / 확신 높음: 유효한 정반대 좌표의 거리가 NaN이다
...
- 최소 개선: 수학적으로 [0,1]인 중간값을 그 구간으로 정규화한 뒤 각도를 계산한다. 기존 antipode 테스트에 해당 입력과 finite 단언을 포함한다. 지리 라이브러리 추가는 불필요하다.

### L4 — P2 / 확신 높음: ByteUtil이 자신이 허용한 소수 바이트를 잃는다
...
- 최소 개선: 최종 B 잔여량을 소수까지 보존한다. 기존 byte spec의 왕복 표에 양·음수 소수 바이트와 1B 미만을 포함한다. 입력을 정수로 축소하는 것은 별도 계약 결정이므로 단순 수정을 이유로 제한하지 않는다.

### L5 — P2 / 확신 높음: NaN TTL이 영구 저장으로 바뀐다
...
- 최소 개선: 이미 오류 문구로 요구하는 유한 정수 TTL을 set 이전에 검증한다. withLock도 동일한 정수 계약과 검사 조건이 어긋나므로 함께 점검한다. `incrementWithExpiry`는 INCR 이후 PEXPIRE를 실행하므로 TTL 검사 실패는 Lua 실행 전에 해야 부분 쓰기를 피한다.
- 테스트 공백: 기존 cache spec은 음수·0·정상 양수만 다룬다.

### L6 — P2 / 확신 높음: fixture 부분 생성 실패가 열린 자원을 남긴다
...
- 최소 개선: 각 fixture가 취득한 자원을 즉시 소유하고 후속 설정이 실패하면 catch/finally에서 모두 close한다. 여러 close 중 하나의 실패가 나머지 정리를 막지 않도록 settle하고 원래 setup 오류를 유지한다. 기존 파일 안의 실패 회귀 검사로 충분하다.

## 낮은 우선순위의 확정 값·타입 결함

### L7 — P3 / 확신 높음: BaseConfigService 반환 타입이 런타임에서 지켜지지 않는다
...
- 최소 개선: getter의 허용 입력 타입을 검사한 뒤 변환하고, string getter는 실제 string만 반환한다. 기존 config spec에 null/boolean/number 경계를 넣는다.

### L8 — P3 / 확신 높음: nullable transform 입력을 non-null로 잘못 선언한다
...
- 최소 개선: transform 인자의 타입에서 undefined만 제외한다. null을 건너뛰는 런타임 변경은 기존 null 복사 계약을 바꾸므로 하지 않는다. 현재 앱의 호출은 transform을 쓰지 않아 API 영향은 없다.

### L9 — P3 / 확신 높음: isEqual이 동일 객체를 다르다고 할 수 있다
...
- 최소 개선: 동일 참조/값의 비교는 Object.is로 먼저 끝낸다. 필요하면 서로 다른 객체의 getter 관찰 횟수도 별도 확인한다. Temporal/Map/Set/cycle 처리를 제거하거나 기존 선택을 되돌릴 근거는 아니다. 현재 앱의 Require.equals는 수량 비교여서 직접 영향은 없다.

### L10 — P3 / 확신 높음: pick/pickBy가 __proto__라는 데이터 키를 잃는다
...
- 최소 개선: 선택한 entry를 Object.fromEntries 또는 own data property 정의로 복사한다. 기존 pick/pickBy spec에서 키 보존을 확인한다.

### L11 — P3 / 확신 높음: Require.equals의 진단 메시지가 원래 불변식 오류를 바꾼다
...
- 최소 개선: 비교 가능한 값을 진단 가능한 표현으로 남기되 JSON 직렬화에만 의존하지 않는다. Node inspect 같은 기존 런타임 수단이면 충분하다. 현재 호출은 주로 수량이며 실제 현재 API 영향은 없다.

### L12 — P3 / 확신 높음: TimeUtil의 아주 작은 소수 출력은 parser가 읽지 못한다
...
- 최소 개선 후보: 허용하는 number 출력과 parser의 지수 표기 정책을 일치시킨다. 현재 밀리초 timestamp/TTL 용도에서 sub-nanosecond 값은 사용하지 않으므로 P3이며 새 정밀도 기능을 우선 도입할 이유는 없다.
