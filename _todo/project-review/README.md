...
최초 검토의 기준 코드는 main `1510689e`다. 하위 보고서의 파일 수·행 번호·검증 기록은 당시 상태이며, 현재 작업 범위는 아래 미완료 목록을 따른다. 가이드 개정안은 루트 README와 `docs/`에 반영됐다.
...
| 우선순위 | 내용                                                      | 근거와 최소 범위                                                                     |
| -------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 먼저     | benchmark와 나머지 장시간 race의 5분 token 수명           | [테스트 T4](tests.md). `purchase-double-spend`의 반복별 갱신은 완료; 액세스 TTL 유지 |
...
인프라 초기화를 제외한 AtoZ 본 검증을 로컬에서 통과했다. API 402개·common 528개는 커버리지 100%를 유지했고, testing 29개·브라우저 E2E 18개·API 문서 76개도 통과했다. GitHub Actions에서 수정본을 다시 실행한 결과는 아니다.

별도로 4개 복제본에서 SSE 100개 client × 10개 saga의 이벤트 1,000개 전달을 1회 검증했다. `replica-chaos`는 기본 시간·부하·오류율 기준으로 통과했고, 복구 후 복제본 4개 응답과 전체 오류율 0.056%를 관측했다. SSE 기본 150회 반복과 전체 race 묶음을 실행한 결과는 아니다.
