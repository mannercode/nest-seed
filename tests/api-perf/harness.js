 ...
 * 튜닝 사이클마다 반복해서 사용하는 도구이고, 원본 결과는 `_output/perf/<scenario>-<timestamp>.json`에 저장한다.
 *
 * 환경 변수:
...
// `keepAlive: true`로 NGINX keep-alive 풀을 그대로 사용한다.
// 워커마다 각자의 Agent를 따로 두어 소켓 경합을 피한다.
// 그래야 측정 대상인 ioredis와 NGINX 풀의 큐 동작이 가려지지 않는다.
// TCP 계층이 병목이 되어선 안 된다.
function makeAgent() {
...
function buildRequestFactory(scenario) {
    ...
    if (scenario === 'user-read') {
        ...
        // 이 시나리오가 재는 것은 Mongo 읽기 처리량이 아니라 인증 실패 응답의 처리량이다.
        // 인증 없이 Mongo 읽기를 측정하고 싶으면 `theater-read`나 `movie-read` 시나리오를 사용한다.
        return () => ({ method: 'GET', path: '/users?take=50', body: null, expectStatus: 200 })
    }
    if (scenario === 'theater-write') {
        // `POST /theaters`는 가드가 없다.
        // 순수 Mongo 쓰기 + majority commit 비용만 잰다.
        // 본문은 작지만 중첩 검증이 있어서 무시할 수준으로 보기 어렵다.
        return (workerId, seq) => ({
            method: 'POST',
            path: '/theaters',
            body: {
    ...
    if (scenario === 'theater-read-name-filter') {
        // 검색어를 좁게 설정해 매치 수를 거의 0에 맞춘다.
        // 부분 문자열 정규식은 일반 인덱스를 활용하지 못해 컬렉션 전체를 스캔한다.
        // 그 비용을 단독으로 측정하기 위한 시나리오이다.
        return () => ({
            method: 'GET',
