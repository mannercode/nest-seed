...
| `api-benchmark`      | 같은 머신의 이전 실행과 RPS·latency 비교                             | `npm run benchmark:api`      | JSON과 HTML dashboard               |
...
```bash
npm run e2e:list                 # 서버를 띄우지 않고 테스트 이름만 확인
npm run e2e:ui                   # 테스트 선택·실행·디버깅
...
npm run race -- ticket-holding-race  # 시나리오 하나 실행
```
