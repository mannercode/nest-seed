 ...
 * 나눈다. 각 쌍마다 같은 사용자 그룹이 동시에 선점을 시도하고, 다른 그룹도 같은 시점에
 * 각자의 쌍으로 요청을 보낸다. 그룹 한 개당 결과는 정확히 한 사용자가 204를 받고,
 * 나머지는 409이다.
 *
 * 사용자 계정은 처음 한 번 만들어 회차 사이에 그대로 다시 사용한다. 이미 선점된 티켓은
 * 같은 락 키로 다시 경쟁시킬 수 없으므로 매 회차마다 새 티켓만 만든다.
 *
 * 어떤 그룹의 204 응답 수가 1이 아니거나, 5xx가 발생하거나, 응답한 복제본 수가 두 개보다
 * 적으면 실패로 본다.
 */
