...
- `WORKSPACE_ROOT` 환경 변수를 자동 설정합니다.
...
- `tests/repeat`: 단위 테스트 반복 실행 유틸리티
...
E2E 스크립트에서 현재 워크스페이스 위치를 알기 위해 사용합니다.
Dev Container에서는 자동 설정되며, 로컬에서는 직접 설정해야 합니다.
...
각 스위트마다 인프라/앱을 재시작합니다.

### 테스트 반복 실행

불안정한 테스트를 잡기 위해 여러 번 반복 실행할 수 있습니다.

- 로컬: `tests/repeat/run.sh`
...
- `transformIgnorePatterns`: ESM 의존성(chalk)을 예외 처리합니다.
...
- E2E 실패 (`WORKSPACE_ROOT` 없음): `WORKSPACE_ROOT=$(pwd)`를 붙여 실행하세요.
