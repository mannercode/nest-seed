...
리프레시 토큰은 Redis에서 관리하고 사용할 때마다 회전한다. 일반 로그아웃은 해당 세션을, 전체 로그아웃·비밀번호 변경·계정 삭제는 그 계정의 기존 리프레시 세션을 회수한다. 액세스 권한의 남은 수명은 짧은 만료 시간으로 제한하며, 즉시 회수가 필요한 서버 세션 모델은 사용하지 않는다.
