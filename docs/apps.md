...
테스트 제목을 조건과 결과로 쓰는 프로젝트 공통 규칙은 [컨벤션 §5](reference/conventions.md#5-테스트-문장은-조건과-결과를-이어-읽게-쓴다)가 소유한다.
...
산출물은 token·password·presigned URL 등을 가리지만 공개 문서 호스팅 대상은 아니다. fixture와 요청 본문에 실제 secret·고객 데이터를 넣지 않는다. 실행 명령은 [README](../README.md#3-api-레퍼런스)가 소유한다.
