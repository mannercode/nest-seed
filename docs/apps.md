...
예상 가능한 도메인 실패는 모듈의 `errors.ts`에 코드·사람이 읽을 메시지·문맥 필드를 가진 객체로 정의하고, 그 규칙을 소유한 서비스가 적절한 공통 예외에 담아 던진다. Gateway에서 별도 오류 계층으로 번역하지 않으며, controller에서 도메인 조건을 다시 만들지 않는다.
