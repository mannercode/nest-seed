...
### 메시지 — Core NATS와 JetStream의 선을 그어 둔다
...
Core Service는 도메인 이름, Application Service는 조합하는 유스케이스 이름을 쓴다. 파일 경로는 디렉터리면 `Dir`, 파일까지 포함하면 `Path`로 끝나게 해 호출 측이 추가 `join`을 해야 하는지 이름에 드러낸다.
...
`apps/api/api-docs/*.spec`는 bash와 curl로 작성한 성공 흐름의 API 계약이다. 실제 요청을 보내지 못하는 정적 endpoint 카탈로그 대신, 실행해 요청·응답 예시와 브라우징 가능한 요약을 만든다. 이 선택의 이유는 [설계 결정의 거부 도구](reference/decisions.md#10-명시적으로-거부한-도구)에 있다.
...
산출물은 token·password·presigned URL 등을 가리지만 공개 문서 호스팅 대상은 아니다. fixture와 요청 본문에 실제 secret·고객 데이터를 넣지 않는다. 실행 명령은 [README](../README.md#api-레퍼런스)가 소유한다.

## console·user-app — 최소 데모

두 Next.js 앱은 access/refresh token을 HttpOnly cookie에 보관하고, access token 만료 시 회전한 뒤 원래 요청을 한 번 재시도하는 BFF 계약을 공유한다. BFF는 편의상 catch-all proxy이므로 최종 인가 경계는 API guard다. 두 앱이 새 기능의 기준 구현이라고 가정하지 않는다.

운영에서 proxy IP를 신뢰하려면 BFF와 API 앞의 edge가 외부 헤더를 재구성하고 직접 접근을 차단해야 한다. 해당 보안 경계는 [deploy 문서](deploy.md#프런트엔드-bff와-클라이언트-ip-경계)가 소유한다.
