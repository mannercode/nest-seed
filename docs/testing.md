...
`apps/api` 통합 테스트는 devcontainer가 띄운 MongoDB Replica Set, Redis Cluster, MinIO, NATS, Temporal을 재사용한다. `libs/common` 테스트는 Testcontainers와 Temporal local test environment로 필요한 인프라를 직접 시작한다. 콘솔 e2e는 Playwright가 API와 콘솔을 빌드해 띄운 뒤 브라우저에서 흐름을 검증한다.
...
libs의 테스트 스위트는 스위트별 `createXxxFixture()` 팩토리(예: `createRedisModuleFixture`)로 격리된 컨텍스트를 만들고, `apps/api` 통합 테스트는 공용 `createAppTestContext()`(`src/__tests__/integration/helpers`)를 함께 쓴다. 어느 쪽이든 필요한 NestJS 모듈과 HTTP 클라이언트를 묶어서 반환하고, 테스트가 끝나면 `teardown()`으로 자원을 정리한다.
...
콘솔 e2e는 `tests/console-e2e`의 Playwright 설정이 `apps/api`와 `apps/console`을 빌드해 실행한 뒤 브라우저에서 로그인, 영화 등록 흐름을 검증한다. 개발 중 이미 서버가 떠 있으면 재사용한다. 서버를 빌드해 띄우는 만큼 무거워서 기본 `npm test`에는 넣지 않았다 — `npm run e2e -w tests/console-e2e`로 직접 실행하거나 `npm run atoz`에 포함되어 돈다.
...
| `showtime-overlap-race.js` | 겹치는 시간대 사가 동시 요청 → 분산 락으로 1개만 성공, 나머지는 실패       |
