...
테스트 코드는 사람이 *읽기 위한* 문서이기도 하다. 그래서 영어 메서드 이름과 한글 주석을 함께 써서, 코드만 보고도 시나리오가 자연스럽게 읽히도록 한다.
...
PATCH나 DELETE처럼 상태를 바꾸는 API는 *응답이 올바른가* 와 *DB에 반영되었는가* 를 다른 `it` 으로 분리한다. 두 검증이 한 함수에 섞이면 어느 쪽이 깨졌는지 한 눈에 알기 어렵다.

---

## 3. Dynamic Import — 왜 필요한가

테스트마다 격리된 환경을 만들기 위해, 각 테스트는 고유한 `TEST_ID` 를 부여받고 그 ID로 자기만의 MongoDB 데이터베이스와 S3 버킷을 만든다. 이게 동작하려면 fixture가 *매 테스트마다* `TEST_ID` 를 다시 읽어 와야 한다.
...
단일 프로세스 테스트로는 *여러 replica가 동시에 같은 자원을 건드릴 때* 발생하는 race를 재현하기 어렵다. 그런 race는 4-replica docker compose 스택을 직접 띄우고 블랙박스로 두드려 봐야 잡힌다.
...
| `ticket-holding-race.js`   | 동시 hold-tickets → Redis SET NX로 1 × 200 + 나머지 × 409                     |
