...
테스트 코드는 사람이 _읽으라고 만든_ 문서이기도 하다. 코드 식별자를 가리키는 자리에는 영어를 그대로 쓰고, 시나리오를 풀어 쓰는 자리에는 한글 문자열을 그대로 넣어서, 코드만 봐도 흐름이 자연스럽게 읽히게 한다.
...
각 테스트 스위트는 `createXxxFixture()` 팩토리로 자기만의 격리된 컨텍스트를 만든다. fixture 는 그 스위트에 필요한 NestJS 모듈, HTTP 클라이언트, 시드 데이터 등을 묶어서 돌려주고, 끝날 때 `teardown()` 으로 자원을 정리한다.
...
테스트마다 격리된 환경을 만들기 위해, 각 테스트는 자기만의 `TEST_ID` 를 받고 그 ID 로 자기만의 MongoDB 데이터베이스와 S3 버킷을 만든다. 이게 돌아가려면 fixture 가 _테스트가 시작될 때마다_ `TEST_ID` 를 새로 읽어 와야 한다.

문제는 보통 쓰는 import 방식에 있다. 파일 맨 위에 `import { createUsersFixture } from './users.fixture'` 라고 써 두면, 그 모듈은 맨 처음 한 번만 평가되고, 그때 캡처한 `process.env.TEST_ID` 가 그 시점의 값으로 굳어 버린다. 그러면 모든 테스트가 같은 DB 를 가리키게 돼서 격리가 깨진다.
...
한 프로세스 안에서 도는 테스트로는 _여러 replica 가 동시에 같은 자원을 건드릴 때_ 생기는 race 를 재현하기 어렵다. 그런 race 는 4-replica docker compose 스택을 직접 띄우고 블랙박스로 두드려 봐야 잡힌다.
...
CI 는 [test-stability.yaml](../.github/workflows/test-stability.yaml) 에서 같은 시나리오를 50 회 반복 실행하면서, 결과가 얼마나 들쭉날쭉한지 누적해 본다. race 코드는 한 번 통과했다고 안전하다고 볼 수 없으니, 통계로 확인하는 셈이다.
