...
- [../tests/api-race/](../tests/api-race/) — API 컨테이너 4개로 실행하는 분산 레이스 시나리오. [tests 문서](tests.md#api-race--분산-레이스-시나리오)에서 자세히 설명한다.
- [../tests/api-perf/](../tests/api-perf/) — 같은 4-replica 스택을 대상으로 하는 성능 측정. [tests 문서](tests.md#api-perf--성능-측정)에서 자세히 설명한다.
