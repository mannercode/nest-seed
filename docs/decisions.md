...
건너뛰어도 사용자에게 영향이 없다면 `withLock`이면 충분하다. 반면 사가의 검증-삽입 경쟁처럼 요청을 버릴 수 없는 곳에서는 `withLockBlocking`으로 순서대로 처리한다. 구매 흐름도 같은 락으로 직렬화하지만, 이중 판매 방지 자체는 티켓의 원자 조건부 전이(Available→Sold)가 보장한다 — 락은 패자가 결제 생성·보상까지 가는 낭비를 줄이는 최적화다.
...
시드는 포크한 사람이 그대로 본보기로 삼는 코드다. "대체로 높은" 커버리지는 어느 줄이 검증 밖인지 따로 추적해야 하지만, 100%는 그 추적 자체를 없앤다 — 테스트 없는 코드가 들어오면 게이트가 바로 막는다. 도달하기 어려운 분기는 커버리지를 낮출 이유가 아니라 코드를 단순하게 고치라는 신호로 쓴다.

### 검토했던 대안

- **임계값 80~90%** — 남는 10~20%가 어디인지 아무도 추적하지 않게 된다. 시드 규모에서는 100%를 유지하는 비용이 크지 않다.
