...
이 문서는 분산 협력 도구를 **왜 골랐는지** 설명한다. 비슷한 일을 할 수 있지만 **쓰지 않기로 한 도구**도 함께 정리한다. 각 도구를 어디서 쓰는지는 [architecture.md §2](architecture.md#2-분산-협력--msa-ready-monolith)에 있다.
...
- **sticky session(NGINX)** — 클라이언트를 한 컨테이너에 계속 붙이는 방법이다. 하지만 이벤트를 만드는 워커가 다른 컨테이너에 있을 수 있으므로 근본적인 해결책이 아니다.
