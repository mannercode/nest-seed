...
> 과거 msa 시드에 해당하던 결정 (NATS, Temporal, Kong) 은 [docs/msa-archive.md](msa-archive.md) 로 옮김. 이 문서는 현재 api 에 적용되는 결정만 유지한다.
...
api 는 프로덕션에서 N replica 로 수평 확장된다. 프로세스 로컬 `EventEmitter2` 는 이벤트가 발행된 replica 의 구독자에게만 전달되므로, 다른 replica 에 붙은 SSE 클라이언트는 상태 변화를 누락한다. showtime-creation 의 saga 진행 상황을 클라이언트에 스트리밍하려면 cross-replica 전달이 필수다.
