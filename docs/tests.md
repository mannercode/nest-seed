...
필수 AtoZ는 기본 회귀를 실행하고, Stability는 동일한 AtoZ를 반복해 브라우저·API 문서·인프라 복구까지 간헐적인 실패를 찾는다. 매 회차 인프라를 초기화하고 100% 커버리지 기준도 유지한다. 반복은 여러 러너에 나누며, 각 러너는 첫 실패에서 중단한다. API Race의 경쟁·장애 시나리오는 별도 CI로 반복한다. 횟수·스케줄은 [CI 파일](../.github/workflows/)이 소유한다. 단발 통과도 반복 통과도 모든 경쟁·장애가 사라졌다는 증명은 아니다.
