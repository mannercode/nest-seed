...
코드는 하나의 `apps/api` 안에 있다. 하지만 배포할 때는 API 컨테이너를 **기본 4개** 띄운다. NATS와 Temporal 같은 분산 인프라도 함께 사용한다.
...
현재 [showtimeCreationWorkflow](../apps/api/src/services/application/showtime-creation/worker/workflow.ts) 워크플로가 _validate → create → emit_ 흐름을 담당한다. 중간에 실패하면 catch 블록에서 실패 이벤트를 발행하고 필요한 보상 작업을 수행한다.
