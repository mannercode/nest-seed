...
SoLA는 원래 마이크로서비스를 염두에 둔 원칙입니다. 마이크로서비스에서는 서비스가 서로 다른 프로세스로 떠 있습니다. 그래서 같은 계층끼리 직접 부르기 어렵고, 여러 서비스를 묶는 일은 그 위의 오케스트레이터나 게이트웨이가 맡습니다.
...
- 여러 단계를 거치는 작업이 중간에 실패했을 때 앞 단계 작업을 되돌려야 하는 상황
...
| 중간 실패 시 되돌려야 하는 작업    | Temporal workflow + activities | 저장·재시도·보상 처리             |
...
- `withLock(key, ttl, fn)`은 이미 락이 잡혀 있으면 바로 `{ran: false}`를 돌려주고 끝냅니다. 같은 일이 동시에 여러 번 들어와도 한 번만 실행하면 충분한 곳에 사용합니다. 예를 들어 API 컨테이너 4개가 같은 cron을 동시에 시작하더라도, 그중 하나만 실제 작업을 하면 됩니다.
- `withLockBlocking(key, ttl, fn, {pollMs, waitMs})`은 락이 풀릴 때까지 짧은 간격으로 다시 시도합니다. 요청을 버리지 않고 모두 처리하되, 한 번에 하나씩만 처리해야 할 때 사용합니다. 중복 구매 차단이나 saga의 검증-삽입 경쟁처럼, 그냥 넘기면 사용자 요청을 실패시켜야 하는 곳에 맞습니다.
...
오래 걸리거나 여러 단계를 거치는 작업은 Temporal workflow로 작성합니다. Temporal은 workflow 실행 기록을 서버에 저장합니다. 그래서 워커가 죽어도 다른 워커가 같은 workflow를 이어서 처리할 수 있습니다. 각 단계의 재시도 규칙도 코드로 적어 둘 수 있습니다.
