...
이 시드의 핵심은 두 가지입니다. 하나는 **SoLA 계층 분리**이고, 다른 하나는 **분산 환경에서 안전하게 협력하는 구조**입니다. 코드만으로는 의도를 알기 어려운 부분을 이 문서에 정리합니다.
...
모듈끼리 자유롭게 서로를 부르게 두면 시간이 지날수록 순환 참조가 생깁니다. 처음에는 A만 B를 부르더라도, 기능이 늘면 B도 A를 부르게 되기 쉽습니다. 그러면 두 모듈은 사실상 하나로 묶이고, 한쪽 수정이 다른 쪽까지 흔듭니다.

### 1.2. 해결책: 같은 계층끼리도 직접 부르지 않습니다

흔한 레이어드 아키텍처는 위 계층이 아래 계층을 부르는 방향만 제한합니다. 같은 계층 안에서는 서로 부르게 두는 경우가 많습니다. 하지만 그렇게 두면 같은 계층 안에서 순환 참조가 다시 생깁니다.
...
Core Service 하나로 처리할 수 있는 API라면 컨트롤러에서 Core를 바로 호출합니다. Application 계층을 억지로 끼워 넣지 않습니다. 여러 Core를 함께 써야 하는 유스케이스에서만 Application Service를 만듭니다.
...
SoLA는 원래 마이크로서비스를 염두에 둔 원칙입니다. 마이크로서비스에서는 서비스가 서로 다른 프로세스로 실행됩니다. 같은 계층끼리 직접 부르기 어렵고, 여러 서비스를 묶는 일은 그 위의 오케스트레이터나 게이트웨이가 맡습니다.

모놀리스에서도 이 규칙을 모듈 단위로 적용해 두면, 나중에 특정 모듈을 독립 서비스로 떼어내기 쉽습니다. 다른 모듈과 직접 엮여 있지 않으므로 경계만 끊어낼 수 있습니다.

---

## 2. 분산 협력 — MSA 준비형 모놀리스

코드는 하나의 `apps/api` 안에 있습니다. 하지만 배포할 때는 API 컨테이너를 **기본 4개** 띄웁니다. NATS와 Temporal 같은 분산 인프라도 함께 사용합니다.
...
각 도구를 고른 이유와 검토한 대안은 [decisions.md](decisions.md)에 있습니다. 이 문서에서는 도구를 어디에 어떻게 쓰는지에 집중합니다.

### 2.1. 분산 락 — `cache.withLock`와 `cache.withLockBlocking`

분산 락은 두 형태로 나누었습니다. 기준은 _이미 다른 요청이 같은 일을 처리 중일 때 이번 요청을 어떻게 다룰 것인가_ 입니다.
...
- `withLockBlocking(key, ttl, fn, {pollMs, waitMs})`은 락이 해제될 때까지 짧은 간격으로 다시 시도합니다. 요청을 버리지 않고 모두 처리하되, 한 번에 하나씩만 처리해야 할 때 사용합니다. 중복 구매 차단이나 사가의 검증-삽입 경쟁처럼, 그대로 넘기면 사용자 요청을 실패시켜야 하는 곳에 맞습니다.
...
NestJS의 `EventEmitter2`는 같은 프로세스 안에서만 이벤트를 전달합니다. API 컨테이너가 4개라면 이것만으로 부족합니다. 예를 들어 Temporal 워커가 사가 진행 상황을 만들었는데, SSE 클라이언트가 다른 컨테이너에 붙어 있을 수 있습니다. 이때 이벤트는 컨테이너 밖으로 나가야 합니다.

`NatsPubSubService`는 NATS subject 기반 pub/sub을 감싼 서비스입니다. 같은 subject를 구독하는 모든 컨테이너에 이벤트를 보냅니다. 큐 그룹 옵션을 쓰면 같은 그룹 안에서 한 컨테이너만 이벤트를 받게 할 수도 있습니다.

지금은 showtime-creation 사가의 상태 변화가 이 경로를 탑니다. 사가가 상태를 NATS에 발행하면 모든 컨테이너의 구독 핸들러가 그 이벤트를 받습니다. 각 핸들러는 이벤트를 로컬 RxJS Subject로 넘기고, SSE 컨트롤러는 자기 컨테이너에 붙은 클라이언트에게 흘려보냅니다.

### 2.3. Saga 오케스트레이션 — Temporal

오래 걸리거나 여러 단계를 거치는 작업은 Temporal 워크플로로 작성합니다. Temporal은 워크플로 실행 기록을 서버에 저장합니다. 그래서 워커가 종료되어도 다른 워커가 같은 워크플로를 이어서 처리할 수 있습니다. 각 단계의 재시도 규칙도 코드로 적어 둘 수 있습니다.

대신 워크플로 함수는 같은 입력이면 항상 같은 결과를 내야 합니다. `Date.now()`, 랜덤 값, 외부 I/O처럼 실행할 때마다 결과가 달라지는 일은 액티비티로 빼야 합니다. 액티비티가 DB 쓰기나 외부 API 호출을 맡습니다.

현재 [showtimeCreationWorkflow](../apps/api/src/services/application/showtime-creation/worker/workflow.ts) 워크플로가 _validate → create → emit_ 흐름을 담당합니다. 중간에 실패하면 catch 블록에서 실패 이벤트를 발행하고 필요한 보상 작업을 수행합니다.
