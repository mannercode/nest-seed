...
nest-seed에서 말하는 계층은 컨트롤러/서비스/리포지토리처럼 한 서비스 내부의 코드 구조를 뜻하지 않는다. Application, Core, Infrastructure는 하나의 백엔드 서비스를 제공하기 위해 협력하고, View는 그 서비스를 화면 요구에 맞게 소비한다. 이 규칙은 모듈 간 순환 참조를 원천적으로 막아, 결합도 증가와 변경 전파 같은 문제를 예방한다.
...
하지만 Service 계층은 쉽게 넓어진다. 한 도메인의 기본 기능, 여러 도메인을 조합하는 유스케이스, 외부 시스템 호출 전 조립 로직이 모두 Service 안에 쌓이면, 같은 계층 안에서도 호출 그래프가 얽히고 순환 참조가 생긴다.
...
Application, Core, Infrastructure는 단일 서비스를 제공하는 협력 관계다. Application은 여러 Core를 묶어 유스케이스를 만들고, Core는 도메인 기능과 상태를 책임지고, Infrastructure는 외부 시스템 연동을 맡는다. Gateway와 View는 이 서비스 제공 흐름에 끼어드는 계층이 아니라, 이미 제공되는 서비스를 외부 요청에 맞게 소비하는 계층이다. Gateway는 HTTP 요청을, View는 특정 화면 응답을 책임진다.
...
예를 들어 사용자 앱 홈 화면은 영화, 상영시간, 극장 이름을 한 응답에 담아야 한다. 프론트엔드가 Movies, Showtimes, Theaters API를 각각 호출해서 조립할 수도 있지만, 그러면 호출 수가 늘고 화면별 조립 로직이 클라이언트에 흩어진다. 그래서 `view/user-app/home`처럼 화면 단위의 소비자 코드를 백엔드에 둔다.
...
또한 View는 다른 계층이 재사용하는 공용 서비스가 아니다. 특정 화면의 응답 모양을 담는 가장자리 소비자 코드이므로, Application/Core/Infrastructure가 View를 참조하지 않는다.
...
export class ShowtimesHttpController {
    ...
    @Post()
    async requestShowtimeCreation(@Body() body: BulkCreateShowtimesDto) {
        // 영화·극장·상영시간·티켓을 한꺼번에 다뤄야 하므로 Application을 거친다
        return this.showtimeCreationService.requestShowtimeCreation(body)
    }
}
```
