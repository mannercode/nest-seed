...
Application, Core, Infrastructure는 단일 서비스를 제공하는 협력 관계다. Application은 여러 Core를 묶어 유스케이스를 만들고, Core는 도메인 기능과 상태를 책임지고, Infrastructure는 외부 시스템 연동을 맡는다. View는 이 서비스 제공 흐름에 끼어드는 계층이 아니라, 이미 제공되는 서비스를 화면에 맞게 소비하는 계층이다.

의존 규칙은 다음과 같다.

1. 같은 계층 안에서는 서로를 참조하지 않는다.
2. 서비스 제공 쪽은 위 계층만 아래 계층을 참조할 수 있다. 예: Application → Core → Infrastructure.
3. View는 서비스 소비자이므로 화면 응답에 필요한 읽기 API를 호출할 수 있다.
4. 서비스 제공 쪽(Application/Core/Infrastructure)은 View를 참조하지 않는다.
