...
- [런타임 장치의 필요성 검토](runtime-complexity-review.md): 인증·자동 변환·BFF·선점·상영·파일 정리의 단순화 후보와 유지할 분산 보장.
...
`authVersion`은 액세스 토큰의 즉시 회수를 위해 인증 요청마다 계정 상태를 확인하던 장치였다. 액세스 토큰은 5분 만료까지 유효하고 리프레시 세션만 서버에서 회수한다는 선택에 맞춰 제거했다. 이전 검토 기록의 `authVersion` 유지 판단은 현행 계약이 아니다([인증 계약](../apps.md#335-본인-자원은-me로-다룬다)). 다른 장치의 코드 검토 결과와 검증 한계는 [런타임 검토](runtime-complexity-review.md)에 기록했다.
