...
이 문서는 저장소의 테스트를 **파일 단위**로 찾기 위한 색인이다. 테스트를 왜, 어떻게 실행하는지에 대한 설명은 [tests 문서](tests.md)를, 실행 명령과 결과 위치만 빠르게 확인하려면 [`tests/README.md`](../tests/README.md)를 본다.
...
| [`apps/api/src/__tests__/vitest.setup.ts`](../apps/api/src/__tests__/vitest.setup.ts)                       | API test file별 Mongo·S3 연결, test namespace와 JetStream 정리를 연결한다.                                                                              | **유지 · 핵심** |
