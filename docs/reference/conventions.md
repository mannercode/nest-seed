...
요청·응답 DTO는 Zod 스키마를 정의하고 `z.infer`로 타입을 얻는다. 같은 필드를 클래스에 다시 선언하지 않는다. 날짜 복원이 필요한 응답도 해당 DTO 스키마를 재사용한다.

JSON 본문의 문자열·숫자·불리언은 선언한 타입으로만 받는다. 쿼리 문자열의 숫자와 날짜 필드처럼 변환이 필요한 곳에만 변환을 명시한다. 수정 필드의 생략은 기존 값 유지이며, `null`은 모델이 실제로 허용할 때만 받는다.
...
테스트는 각 workspace의 `src/__tests__`에서 실제 모듈·인프라를 연결하는 통합테스트를 기본으로 한다. 단위 테스트는 복잡한 알고리즘처럼 독립 검증의 가치가 있는 로직에 한해 선별하고, 단순 위임이나 내부 함수마다 추가하지 않는다. 브라우저·다중 프로세스 검증은 [외부 스택 테스트](../tests.md)의 경계를 따른다.
...
배포 중 바꿀 필요가 없는 불변식은 사용하는 코드 가까이에 둔다. 모든 값을 env로 옮기면 설정 파일이 두 번째 코드베이스가 된다. 예를 들어 현재 티켓 가격은 API env가 소유하지만, 화면 조회 개수처럼 코드가 소유하는 정책까지 임의로 env에 추가하지 않는다. 개발 환경의 파일 소유권과 주입 시점은 [Dev Container](../devcontainer.md#1-환경-변수는-재생성해야-반영된다)를 따른다.
