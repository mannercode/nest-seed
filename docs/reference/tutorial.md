...
flowchart LR
    ...
    Purchase --> Payment[외부 결제]
```

상영 생성은 티켓 생성을 함께 끝내야 한다. 구매는 결제 시스템에도 효과를 남긴다. 둘 다 여러 단계지만 실패를 되돌릴 수 있는 경계는 다르다. 이 차이를 뒤에서 transaction과 보상으로 나눈다.

유스케이스의 코드 이름은 영어로 맞춘다. `PurchaseService`, `ShowtimeCreationService`처럼 무엇을 끝내는지 드러내고, 도메인 전문가와 사용하는 용어도 합의한다. 현재 앱의 역할은 user·admin이며 최초 admin 생성은 별도의 운영 스크립트가 맡는다.

## 2. 외부 계약을 먼저 그린다

상영 생성의 기본 동선은 영화 선택 → 극장 선택 → 기존 시간 확인 → 생성 요청이다. 관련 API들을 범용 목록 하나의 옵션으로 계속 늘릴 수도 있지만, 특정 유스케이스에서만 쓰는 조합은 namespace로 드러낸다.
...
`ShowtimesService`가 영화를 확인하려고 `MoviesService`를 호출한다고 하자. 나중에 영화 화면에 상영 정보를 붙이면서 Movies가 Showtimes를 호출하면 순환이 생긴다. 두 클래스가 서로를 주입할 수 있게 만드는 것만으로는 책임의 결합이 사라지지 않는다.

SoLA는 같은 계층의 서로 다른 모듈끼리 직접 참조하지 못하게 한다. 상영·영화·극장·티켓을 조합하는 상위 Application을 두고 각 Core는 자기 상태와 규칙을 맡긴다.
...
컨트롤러를 Gateway로 분리한 이유도 같은 경계다. 도메인 모듈이 HTTP 라우팅·인증 주체 추출에 함께 묶이지 않게 한다. View는 그 서비스를 화면에 맞게 소비하는 읽기 조합이며 도메인의 상태 변경을 소유하지 않는다.
...
결제 provider·Redis claim·MongoDB는 하나의 transaction으로 묶이지 않는다. 구매는 외부 효과 전에 `pending` 기록을 남기고 완료·보상을 lease로 실행한다. 완료에는 티켓 판매와 구매 상태·응답 스냅샷을 함께 저장하고, 보상은 해당 구매가 소유한 효과만 해제한다. 프로세스가 종료되면 다른 복제본이 만료 lease를 인수한다. `catch` 안에서만 보상하면 프로세스가 사라진 뒤에는 실행할 코드도 복구 기준도 없다.
