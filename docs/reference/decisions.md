...
- `tools/dev-tools`의 tunnel 정책은 Bash 계약 테스트와 shellcheck로, root `tools/__tests__`의 clean·CI 진단·shell lint 도구는 AtoZ `test:tools`로 검증한다. 여기에는 언어 커버리지 임계치를 붙이지 않는다.
