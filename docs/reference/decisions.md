...
View는 읽기 조합과 화면 정책만 담당한다. 도메인 상태 변경·transaction·보상을 두지 않으며, 다른 계층이 View를 재사용하지 않는다. GraphQL은 현재 화면 수에 비해 스키마·resolver·인가·N+1 설계 비용이 크고, frontend에서의 조합은 같은 로직을 여러 클라이언트로 흩어지게 한다. 정확한 계층 규칙은 [apps 문서](../apps.md#12-view는-화면-전용-서비스-소비자다)가 소유한다.
