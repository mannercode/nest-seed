...
- **root** — `.env.api`의 자격증명으로 Basic 인증. admin 생성·삭제(lifecycle)만 한다.
...
한 컨트롤러에 **서로 다른 역할의 핸들러가 섞이면**(`users`는 `/me`가 본인·`/:id`가 admin, `admins`는 `/me`가 본인·`/:id`가 root) 가드는 클래스가 아니라 **핸들러마다** 붙이고, 모든 핸들러가 같은 역할인 컨트롤러(예: admin 전용 `showtime-creation`)는 클래스 레벨 가드를 쓴다. 클래스 가드를 두면 메서드 가드와 합쳐지는 NestJS 동작 때문인데, 그 이유는 [users.http-controller.ts](apps/api/src/services/gateway/users.http-controller.ts) 머리 주석에 있다.
