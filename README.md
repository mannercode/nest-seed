...
- **Redis 분산 락** — 컨테이너 여러 개가 같은 키를 동시에 처리하는 경쟁을 차단한다 (cron 중복 실행, 사가의 검증-삽입)
- **NATS pub/sub** — 다른 컨테이너에 붙은 SSE 클라이언트에게 이벤트를 전달한다. 큐 그룹 수신도 지원한다
- **Temporal Saga** — 다단계 작업의 실행 기록·재시도·보상을 워크플로로 다룬다
- **원자 조건부 전이** — 티켓 이중 판매를 락이 아니라 상태 조건부 갱신으로 막는다
- **soft delete × unique 인덱스** — 탈퇴한 이메일의 재가입을 복합 인덱스로 허용한다
- **presigned 업로드 + 체크섬** — 신고한 체크섬과 다른 본문은 스토리지가 업로드 자체를 거부한다
- **검증 장치** — 실행 가능한 API 문서, 분산 레이스 하네스, CI에서 레이스 시나리오 50회 반복(test-stability)
- **`npx tunnel`** — 로컬 dev 서버 3종을 임시 공개 https 주소로 노출한다 (OAuth 콜백·웹훅 수신)
