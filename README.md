...
백엔드는 `apps/api`, 관리 콘솔은 `apps/console`에 둔다. 배포할 때는 API 컨테이너 여러 개를 기본으로 띄우고, Redis, NATS, Temporal 같은 분산 도구도 처음부터 함께 쓴다. 그래서 개발은 모놀리스처럼 단순하게 시작하고, 필요해지면 특정 기능을 독립 서비스로 떼어내기 쉽다.
...
```bash
npm test                         # 워크스페이스 테스트 (libs 빌드 자동, console e2e 포함)
bash deploy/test.sh              # Docker Compose로 시작한 뒤 curl 스펙으로 검증
npm run dev                      # libs watch + api watch + console dev 동시 실행
...
`apps/api/src/`의 영화 예매 도메인(Users, Movies, Theaters, Showtimes, Tickets, Bookings, Purchases)을 새 도메인으로 바꾼다. 콘솔 화면은 `apps/console/src/app/` 아래에 있다. 통합 테스트는 `apps/api/src/__tests__/` 아래에 있고, 콘솔 e2e는 `tests/console-e2e/` 아래에 있다. 실행 가능한 API 문서(`apps/api/api-docs/*.spec`)도 함께 수정한다.
...
- 테스트 인프라 (Jest + Testcontainers + curl API 문서 + Playwright console e2e + 분산 레이스 테스트)
- Dev Container 구성
- ESLint 계층 의존성 검증
...
| `PurchaseHttpController`                                      | 구매 처리와 구매 기록 조회. 본문의 `userId`를 그대로 받아 임의 사용자 이름으로 결제할 수 있고, 구매 기록도 인증 없이 조회할 수 있음 |
