...
단일 프로세스 테스트만으로는 여러 API 컨테이너가 동시에 같은 자원을 건드릴 때 생기는 문제를 찾기 어렵습니다. 그래서 API 컨테이너 4개를 Docker Compose로 띄운 뒤, 외부에서 HTTP 요청을 동시에 보내는 테스트를 따로 둡니다.
...
- `.github/workflows/`의 실행 브랜치와 시크릿을 새 저장소에 맞춥니다. `test-atoz.yaml`은 메인 CI이고, `test-stability.yaml`은 분산 레이스 테스트를 여러 번 돌려 안정성을 확인합니다.
...
이 시드는 **인증(`UserJwtAuthGuard`)만 일부 엔드포인트에 적용하고, 인가(role / ownership 검사)는 의도적으로 넣지 않았습니다.** 아래 컨트롤러는 가드 없이 열려 있거나, 인증된 사용자라면 다른 사용자의 데이터도 건드릴 수 있습니다. 실제 서비스로 가져갈 때는 도메인 정책에 맞게 admin 권한, owner-only 검사 등을 추가해야 합니다.
