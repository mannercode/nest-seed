...
2. 로컬 인프라 실행 (MongoDB Replica Set, Redis, NATS, MinIO)
    ...
    ```sh
    npm run infras:reset
    ```

    - `infras/local/compose.yml`을 사용하며 `.env`와 `.env.infra`를 함께 읽습니다.
...
4. E2E 테스트 실행
    ...
    - 각 스위트마다 `npm run infras:reset`과 `npm run apps:reset`으로 인프라/애플리케이션을 초기화합니다.
...
3. Dev Container는 `.devcontainer/Dockerfile`(베이스 `node:24-bookworm`)을 사용하며, `postCreateCommand`/`postStartCommand`로 `npm ci` 및 `npm run infras:reset`을 자동 실행합니다.
