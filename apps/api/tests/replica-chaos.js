// 트래픽이 흐르는 중에 API 복제본 4대 가운데 한 대를 강제로 종료합니다. 이 부하
// 테스트는 세 가지를 함께 검증합니다.
...
//   30초      대상 복제본을 `docker kill`로 종료합니다.
//   30~60초   복제본 3 대만으로 트래픽을 계속 받습니다.
//   60초      대상 복제본을 `docker start`로 다시 시작합니다.
//   60~90초   복제본이 healthy가 될 때까지 기다리며 트래픽을 유지합니다.
//   90~150초  복구 후 트래픽. 복제본 4대가 모두 응답해야 합니다.
//
// 다음 중 하나라도 해당하면 실패로 봅니다. 전체 구간 합산 에러율이 1%를
// 넘기면 (nginx 재시도가 종료 구간의 일시 실패를 가려야 합니다),
// 재시작 뒤 60초 안에 healthy가 되지 않으면, 또는 복구 후 구간에서 응답한
// 복제본 수가 4보다 적으면.
