/**
 * 강한 동시 경합 위에서 사용자 이메일 유일성이 깨지지 않는지 검증하는 부하 테스트이다.
 *
 * 한 회차는 서로 다른 이메일 여러 개를 준비하고, 각 이메일마다 동일한 가입 요청을
 * 한꺼번에 보낸다. 모든 그룹을 같은 시점에 보내 NGINX가 요청을 여러 복제본으로
 * 분산하게 한다. 그룹 한 개당 결과는 정확히 한 요청만 201이고 나머지는 409이다.
 * 5xx는 한 건도 없어야 한다.
 *
 * 어떤 그룹의 201 응답 수가 1이 아니거나, 5xx나 예상하지 못한 상태 코드가 나오거나,
 * 모든 응답이 한 복제본에서만 오면 실패로 본다.
 */
...
async function runInner(iteration) {
    ...
    for (const [email, g] of byEmail) {
        if (g.created !== 1) {
            ...
            // 소켓이 끊겼는지, 예상하지 못한 상태였는지 구분한다. 이전에는
            // 일시 실패가 나도 진단 정보 없이 0 x 201만 출력했다.
