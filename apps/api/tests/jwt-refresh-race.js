/**
 * JWT 리프레시 토큰 회전을 강한 동시 경합 위에서 검증하는 분산 부하 테스트이다.
 *
 ...
 *
 * 안전 조건은 단순하다. 같은 토큰 하나로 동시에 들어온 리프레시 요청들이 동시에 쓸 수
 * 있는 새 토큰을 두 개 이상 만들면 안 된다. 결과는 둘 중 하나여야 한다. 한 요청만
 * 통과하고 나머지는 401이거나, 회전 경합이 재사용 탐지를 자극해 토큰 묶음 전체가
 * 무효화되어야 한다.
 *
 * 한 회차는 사용자 그룹을 여러 개 만들고 각자 로그인한 뒤, 각 사용자의 리프레시 토큰
 * 하나에 대해 동시 `/users/refresh` 요청을 복제본 여러 대에 걸쳐 보낸다. 모든 그룹을
 * 같은 시점에 보내 NGINX가 요청을 복제본들에 분산하게 만든다.
 *
 * 그룹마다 다음을 확인한다.
...
async function runInner(iteration) {
    // 사용자 그룹 수만큼 사용자를 만든다. 각자 새 리프레시 토큰을 하나씩 들고
    // 출발한다.
    const tokens = await Promise.all(
        Array.from({ length: USER_GROUPS }, (_, g) => createAndLogin(`${iteration}-${g}`))
    )

    // 사용자마다 각자의 원래 토큰 하나로 리프레시 요청을 한꺼번에 보낸다. 모든
