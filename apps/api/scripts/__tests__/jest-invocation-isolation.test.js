...
test(
    'two real Jest invocations isolate resources and every output directory',
    { timeout: 120_000 },
    async (context) => {
        const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'nest-seed-jest-isolation-'))
        ...
        context.after(() => {
            ...
            for (const outputDirectory of disposableOutputDirectories) {
                assert.match(
                    outputDirectory,
                    new RegExp(
                        `^${escapeRegex(path.join(apiDir, '_output/jest-runs'))}${escapeRegex(
                            path.sep
                        )}r[a-f0-9]{32}$`
                    )
                )
        ...
        if (failures.length > 0) {
            throw new AggregateError(
                ...
                'Parallel Jest infrastructure probes failed'
            )
        }
...
function runProbe({ barrierDirectory, peerResultPath, resultPath, role, sharedLogDirectory }) {
    return new Promise((resolve, reject) => {
        const child = spawn(
            process.execPath,
            [jestBin, '--config', fixtureConfig, '--runInBand', '--coverage=false'],
            {
                cwd: apiDir,
                env: {
                    ...process.env,
                    API_JEST_RUN_ID: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
                    JEST_ISOLATION_BARRIER_DIRECTORY: barrierDirectory,
                    ...(peerResultPath ? { JEST_ISOLATION_PEER_RESULT_PATH: peerResultPath } : {}),
                    JEST_ISOLATION_RESULT_PATH: resultPath,
                    JEST_ISOLATION_ROLE: role,
        ...
        child.on('close', (code, signal) => {
            if (code === 0) resolve()
                ...
                reject(
                    new Error(`Jest probe ${role} failed (${code ?? signal})\n${stdout}\n${stderr}`)
                )
        })
    })
}
