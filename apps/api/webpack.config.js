...
module.exports = (options) => {
    ...
    return {
        ...
        // `@mannercode/temporal-sandbox`는 Temporal SDK에 절대 경로(`payloadConverterPath`)를 넘기는데, webpack이 인라인하면 그 안의 `__dirname`이 번들 출력 위치로 바뀌어 실제 파일을 잃는다.
        // external로 두면 node_modules의 원래 위치가 보존된다.
        externals: [
            ({ request }, callback) => {
