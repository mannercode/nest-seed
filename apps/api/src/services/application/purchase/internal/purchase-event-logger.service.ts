 ...
 * 복제본이 4개면 이벤트 하나마다 핸들러가 4번 실행된다.
 * 복제본마다 가진 로컬 상태를 모두 갱신해야 할 때 사용한다.
 * 인메모리 캐시 무효화, 핫리로드 설정 갱신, 로컬 메트릭 집계가 그 예이다.
 *
 * 반대로 같은 이벤트를 한 복제본만 처리해야 하는 일(알림 발송, 원장 기록)은 큐 그룹을 사용한다.
 * `PurchaseNotificationService`가 그 예이다.
 */
@Injectable()
