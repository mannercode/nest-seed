...
// 겹침 검사는 상영 시간을 10분 단위 슬롯으로 펼쳐 비교합니다. 이 값은 환경별
// 튜닝 값이 아니라 도메인 정책이므로 코드 상수로 고정합니다.
...
const iterateTimeslots = (
    ...
    // 끝 시각은 포함하지 않습니다. A가 12:00에 끝나면 12:00 시작하는 B와
    // 곧바로 이어져도 충돌이 아닙니다. 청소 시간 같은 간격이 필요하면
    // 호출자가 입력 단계에서 그 간격을 설정해야 합니다.
