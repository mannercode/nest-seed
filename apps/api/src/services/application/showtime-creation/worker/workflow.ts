...
// 검증·삽입은 멱등하지 않으므로 재시도하지 않는다. 검증·삽입 락이 해제될
// 때까지 최대 10분(`waitMs`)을 기다리므로 타임아웃을 그보다 길게 둔다.
const { validateAndCreate } = proxyActivities<ReturnType<ShowtimeCreationActivities['bind']>>({
    startToCloseTimeout: '15 minutes',
    retry: { maximumAttempts: 1 }
})

// NATS 발행과 보상 삭제는 멱등하다(같은 `sagaId`의 같은 `status` 이벤트를
// 두 번 받아도 SSE 검증은 some()으로 첫 매치를 보고, deleteBySagaIds는 두 번
// 호출해도 결과가 같다). Temporal 워커의 일시적 작업 수신 지연이 5분짜리
// SSE 대기 시간 안에서 회복되도록 짧은 타임아웃과 빠른 재시도로 둔다.
...
export async function showtimeCreationWorkflow(
    ...
    // 값을 사용한다. enum 상수를 가져오지 않고 문자열을 직접 적는 이유는,
    // 워크플로 코드가 샌드박스 안에서 실행되기 때문이다. 형제 모듈을 따라
    // `@nestjs/common`의 데코레이터가 함께 포함되면 번들이 실패한다.
