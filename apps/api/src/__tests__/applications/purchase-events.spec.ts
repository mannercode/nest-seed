...
describe('PurchaseEvents subscribers', () => {
    ...
    it('delivers ticketPurchased to both notification and logger', async () => {
        ...
        await fix.events.emitTicketPurchased({ customerId, ticketIds })
        ...
        expect(fix.notification.received).toEqual([{ customerId, ticketIds }])
        const purchased = fix.logger.entries.filter((e) => e.kind === 'purchased')
        expect(purchased).toEqual([{ event: { customerId, ticketIds }, kind: 'purchased' }])
    })
    ...
    it('delivers ticketPurchaseCanceled only to the logger', async () => {
        ...
        await fix.events.emitTicketPurchaseCanceled({ customerId, ticketIds })
        ...
        expect(fix.logger.entries).toEqual([
            { event: { customerId, ticketIds }, kind: 'canceled' }
        ])
    })
})
