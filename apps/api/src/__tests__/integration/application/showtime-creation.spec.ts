...
describe('ShowtimeCreationService', () => {
    ...
    describe('POST /showtime-creation/showtimes', () => {
        ...
        it('기존 상영 시간과 겹치면 충돌 목록과 함께 failed를 보고한다', async () => {
            ...
            // 새 12:00-12:30 은 기존 12:00-13:30 과 시간이 겹치므로 충돌이다.
            // 새 16:00-16:30 과 기존 16:30-18:00, 새 20:00-20:30 과 기존
            // 18:30-20:00 은 한쪽이 끝나는 시각에 다른 쪽이 시작한다. 끝
        ...
        it('기존 상영 시간이 새 범위보다 먼저 시작했어도 끝이 겹치면 충돌로 보고한다', async () => {
            // 기존 09:00-11:00 은 새 요청의 시작 시각(10:00) 보다 일찍 시작
            // 했다. 시작 시각만 보면 새 범위 바깥이지만, 끝 시각이 새 범위와
            // 겹치므로 충돌로 봐야 한다.
            const [initialShowtime] = await createShowtimes(fix, [
                {
