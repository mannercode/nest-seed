...
/**
 * 통합 테스트용 NestJS 앱을 부팅합니다. `AppModule` 전체를 가져오므로 운영
 * 환경과 같은 모듈 그래프 위에서 검증합니다. 테스트 fixture는 추가
 * `imports` 나 컨트롤러를 따로 선언할 필요가 없습니다. mock으로 바꾸거나
 * 가드를 끄려면 `overrideProviders`와 `ignoreGuards` 옵션을 그대로 사용합니다.
 */
