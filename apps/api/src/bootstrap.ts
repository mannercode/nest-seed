...
export async function bootstrap() {
    ...
    // 복제본을 구별하기 위한 식별자입니다. Docker 환경에서는 hostname이 컨테이너
    // ID와 같아서 복제본마다 다른 값이 나옵니다. 분산 부하 테스트에서 어느 복제본이
    // 응답했는지 확인할 때 사용합니다. `docker ps`와 NGINX upstream 로그에 이미
    // 같은 값이 보이므로 외부에 노출해도 추가 위험은 작습니다.
