...
export class StorageFilesProxy {
    ...
    saveFiles(createDtos: StorageFileCreateDto[]): Promise<StorageFileDto[]> {
        return getProxyValue(
            this.service.send('infrastructures.storage-files.saveFiles', createDtos)
        )
    }
    ...
    getStorageFile(fileId: string): Promise<StorageFileDto> {
        return getProxyValue(
            this.service.send('infrastructures.storage-files.getStorageFile', fileId)
        )
    }
    ...
    deleteStorageFile(fileId: string): Promise<boolean> {
        return getProxyValue(
            this.service.send('infrastructures.storage-files.deleteStorageFile', fileId)
        )
    }
}
