...
애플리케이션과 테스트에서 공용으로 사용하는 설정 파일이다. 프로젝트 ID, 포트, MongoDB/Redis/MinIO 접속 정보, 로그 설정 등이 포함된다.
...
- **Debug App**: `npm run debug`를 실행하여 Watch 모드로 앱을 디버깅한다.

### 작업 (`.vscode/tasks.json`)

| Task             | Description                                       |
| ---------------- | ------------------------------------------------- |
| `Run Unit Tests` | `npm run test:unit` 실행.                         |
...
Gateway의 HTTP 컨트롤러에서 `FoosService`를 직접 주입하여 사용한다.
